...
my $make_bed_around_RE_site_pl = '/data/tusers.ds/zhongrenhu/Software/LACHESIS/src/bin/make_bed_around_RE_site.pl';
my $bedtools = '/usr/local/bin/bedtools';
my $samtools = '/data/tusers/zhongrenhu/Software/anaconda3/bin/samtools';
#my $mem = "16G";
...
if    ( uc($extension) eq 'SAM' ) { run_cmd( "$samtools view -bS -@ 32 $SAM -o $head.bam" ); }
...
run_cmd( "$samtools view -@ 32 -F2316 $head.REduced$nodups.bam -o $head.tmp.sam -U $head.tmp.not.sam" );
run_cmd( "$samtools view -H $head.REduced$nodups.bam > $head.tmp.header" );
run_cmd( "cut -f 1 $head.tmp.sam | sort | uniq -c > $head.tmp.ids" );
run_cmd( "grep '1 ' $head.tmp.ids | cut -d ' ' -f 8 > $head.tmp.1.ids" );
run_cmd( "awk '{if(NR==FNR){a[\$1]=1}else{if(\$1 in a){print \$0}}}' $head.tmp.1.ids $head.tmp.not.sam > $head.tmp.1.sam" );
run_cmd( "cat $head.tmp.header $head.tmp.sam $head.tmp.1.sam | samtools view -@ 32 -F2316 -b - > $head.tmp.bam" );
run_cmd( "samtools sort -@ 32 -n -O BAM -o $head.REduced$nodups.paired_only.bam $head.tmp.bam" );
